#Version 1
################################################################################
# MailEdit 1.1: a Python/Tkinter text file editor and component.
...
class TextEditor:                          # mix with menu/toolbar Frame class
    ...
    def onCopy(self):                           # get text selected by mouse,etc
        if not self.text.tag_ranges(SEL):       # save in cross-app clipboard
            showerror('MailEdit', 'No text selected')
        else:
    ...
    def onPaste(self):
        try:
            ...
            showerror('MailEdit', 'Nothing to paste')
            return
    ...
    def onRunCode(self, parallelmode=1):
        ...
        cmdargs  = askstring('MailEdit', 'Commandline arguments?') or ''
        if os.path.exists(thefile):
            filemode = askyesno('MailEdit', 'Run from file?')
        ...
        elif askyesno('MailEdit', 'Text saved in file?'):
    ...
    def onGoto(self):
        line = askinteger('MailEdit', 'Enter line number')
        self.text.update() 
        self.text.focus()
        if line is not None:
            ...
            if line > 0 and line <= maxline:
                ...
                showerror('MailEdit', 'Bad line number')

    def onFind(self, lastkey=None):
        key = lastkey or askstring('MailEdit', 'Enter search string')
        self.text.update()
        self.text.focus()
        self.lastfind = key
        if key:
            ...
            if not where:
                showerror('MailEdit', 'String not found')
            else:
    ...
    def onOpen(self, loadFirst=''):
        doit = self.isEmpty() or askyesno('MailEdit', 'Disgard text?')
        if doit:
            ...
            if file:
                try:
                    ...
                    showerror('MailEdit', 'Could not open file ' + file)
                else:
    ...
    def onSaveAs(self, forcefile=None):
        ...
        if file:
            ...
            try:
                ...
                showerror('MailEdit', 'Could not write file ' + file)
            else:
    ...
    def onQuit(self):
        if askyesno('MailEdit', 'Really quit MailEdit?'):
...
class TextEditorMain(TextEditor, GuiMakerWindowMenu):  # add menu/toolbar maker 
    def __init__(self, parent=None, loadFirst=''):     # when fills whole window
        ...
        self.master.title('MailEdit ' + Version)         # title iff stand alone
        self.master.iconname('MailEdit')                 # catch wm delete button
...
class TextEditorMainPopup(TextEditor, GuiMakerWindowMenu):
    def __init__(self, parent=None, loadFirst=''):     
        ...
        self.popup.title('MailEdit ' + Version) 
        self.popup.iconname('MailEdit')               
    def quit(self):
